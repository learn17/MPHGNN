...
import dgl
import numpy as np
from MPHGNN.utils.project_utils import *
from itertools import chain
import logging
from MPHGNN.global_configuration import global_config

# The main function is to provide preprocessing steps for MPHGNN.
...
def propagate_then_update(g, current_k, inner_k, input_x_dim_dict, target_node_type, squash_strategy, norm=None, squash_even_odd="all", squash_self=True, collect_even_odd="all", diag_dict=None, train_label_feat=None):
...
def Multipers_Local_Semantic_Extractor(g, k, inner_k, alpha, target_node_type, use_input_features, squash_strategy, train_label_feat, norm, squash_even_odd, collect_even_odd, squash_self=False, target_feat_random_project_size=None, add_self_group=False):
    ...
    with torch.no_grad():
        ...
        with g.local_scope():
            ...
            for k_ in range(k):
                ...
                (target_h_list_list_, target_sorted_keys), g = propagate_then_update(g, k_, inner_k, input_x_dim_dict, target_node_type, squash_strategy=squash_strategy, norm=norm, squash_even_odd=squash_even_odd, collect_even_odd=collect_even_odd, squash_self=squash_self, diag_dict=diag_dict, train_label_feat=train_label_feat)
...
def propagate_and_collect_label(hetero_graph, target_node_type, y, train_label_feat):

    label_target_h_list_list, _ = Multipers_Local_Semantic_Extractor(hetero_graph, 
                1, 
                2, 
                0.0,
